package com.simplechat.infrastructure.websocket.handler
...
import com.simplechat.infrastructure.messaging.service.RedisMessageBrokerService
...
class WebSocketMessageHandler(
    ...
    /**
     * 중복 로그인 알림을 특정 사용자 세션들에게 전송
     */
    fun sendDuplicateLoginNotification(userId: Long, duplicateLoginMessage: DuplicateLoginWebSocketMessage): Mono<Void> {
        val userSessions = sessionManager.getSessionsByUserId(userId)
        ...
        val messageJson = try {
            objectMapper.writeValueAsString(duplicateLoginMessage)
        } catch (e: Exception) {
            log.error("Failed to serialize duplicate login message: {}", e.message)
            return Mono.empty()
        }
        
        val notifications = userSessions.map { session ->
            session.send(Mono.just(session.textMessage(messageJson)))
                .doOnSuccess { log.info("Sent duplicate login notification to session: {}", session.id) }
                .doOnError { error -> log.error("Failed to send duplicate login notification to session {}: {}", session.id, error.message) }
     ...
     * 세션 종료 알림을 특정 세션에 전송
     */
    fun sendSessionTerminationNotification(session: WebSocketSession, terminationMessage: SessionTerminatedWebSocketMessage): Mono<Void> {
        val messageJson = try {
            objectMapper.writeValueAsString(terminationMessage)
        } catch (e: Exception) {
            log.error("Failed to serialize session termination message: {}", e.message)
            return Mono.empty()
        }
        
        return session.send(Mono.just(session.textMessage(messageJson)))
            .doOnSuccess { log.info("Sent session termination notification to session: {}", session.id) }
            .doOnError { error -> log.error("Failed to send session termination notification to session {}: {}", session.id, error.message) }
            .onErrorResume { Mono.empty() }
    }
     ...
     * 여러 세션에 세션 종료 알림 전송
     */
    fun sendSessionTerminationNotification(userId: Long, terminationMessage: SessionTerminatedWebSocketMessage, excludeSessionId: String? = null): Mono<Void> {
        val userSessions = sessionManager.getSessionsByUserId(userId)
            .filter { session -> excludeSessionId == null || session.id != excludeSessionId }
        
        if (userSessions.isEmpty()) {
            log.debug("No sessions to terminate for user: {}", userId)
            return Mono.empty()
        }

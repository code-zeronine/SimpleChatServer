package com.simplechat.infrastructure.session
...
class WebSocketSessionManager(
    ...
    /**
     * 중복 로그인 감지: 사용자가 이미 세션을 가지고 있는지 확인
     */
    fun detectDuplicateLogin(userId: Long): DuplicateLoginResult {
        val existingSessions = getSessionsByUserId(userId)

        return if (existingSessions.isNotEmpty()) {
            val sessionInfo = existingSessions.map { session ->
                val metadata = getSessionMetadata(session.id)
                ExistingSessionInfo(
                    sessionId = session.id,
                    chatRoomId = getChatRoomBySessionId(session.id),
            ...
            DuplicateLoginResult(
                ...
                existingSessionCount = existingSessions.size,
    ...
    /**
     * 중복 로그인 알림 메시지를 기존 세션들에게 전송
     */
    suspend fun notifyDuplicateLogin(userId: Long, newLoginInfo: NewLoginInfo, messageHandler: WebSocketMessageHandler) {
        logger.info("Notifying duplicate login for user: {} from IP: {}", userId, newLoginInfo.ipAddress)

        val duplicateLoginMessage = DuplicateLoginWebSocketMessage(
            newLoginInfo = DuplicateLoginInfo(
                loginTime = newLoginInfo.loginTime,
                ipAddress = newLoginInfo.ipAddress,
                userAgent = newLoginInfo.userAgent
            ),
            action = DuplicateLoginAction.NOTIFY_ONLY
        )

        messageHandler.sendDuplicateLoginNotification(userId, duplicateLoginMessage)
            .subscribe(
                { logger.info("Successfully sent duplicate login notifications to user: {}", userId) },
                { error -> logger.error("Failed to send duplicate login notifications to user {}: {}", userId, error.message) }
            )
    }
     ...
     * 중복 로그인 알림 메시지 생성
     */
    private fun createDuplicateLoginNotification(newLoginInfo: NewLoginInfo): String {
        return """
        {
            ...
            "data": {
                "newLoginAt": "${newLoginInfo.loginTime}",
                "ipAddress": "${newLoginInfo.ipAddress ?: "알 수 없음"}",
                "userAgent": "${newLoginInfo.userAgent ?: "알 수 없음"}"
            }
        }
        """.trimIndent()
    }
     ...
     * 사용자의 특정 세션들을 강제 종료
     */
    suspend fun forceLogoutUserSessions(userId: Long, excludeSessionId: String? = null): Int {
        val sessionsToClose = if (excludeSessionId != null) {
            getSessionsByUserId(userId).filter { it.id != excludeSessionId }
        } else {
        ...
        sessionsToClose.forEach { session ->
            try {
                ...
                closedCount++
                logger.info("Force logged out session: {} for user: {}", session.id, userId)
            } catch (e: Exception) {
                logger.warn("Failed to force logout session {}: {}", session.id, e.message)
            }
        }
